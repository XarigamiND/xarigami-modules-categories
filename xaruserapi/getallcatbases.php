 ...
 * @param $args['nodefaultitemtype'] boolean (optional). Prevent using the default itemtype if getitemtype returns values.
...
function categories_userapi_getallcatbases($args)
{
    ...
    if (empty($nodefaultitemtype)) {
        $nodefaultitemtype = FALSE;
    }
    ...
    foreach($modlist as $modinfo) {
        ...
        if (isset($itemtype) && is_numeric($itemtype)) {
            ...
            $modtypes = xarModAPIFunc($modinfo['module'], 'user', 'getitemtypes',
                                     array(), 0);

            if (!$nodefaultitemtype || empty($modtypes)) {
