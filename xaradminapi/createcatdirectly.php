...
function categories_adminapi_createcatdirectly($args)
{
    ...
    // Argument check
    if (!isset($name) || !isset($description) || !isset($point_of_insertion)) {
    ...
    for ($i=1;$i<4;$i++) {
        ...
        if (!$result) {
            $msg = xarML('Category creation failed');
            throw new DataNotFoundException(array($SQLquery[$i],$bindvars[$i]), $msg);
        }
    }
